...
export const WheelsBodySection: React.FC<WheelsBodySectionProps> = ({
  ...
  return (
    <div className="flex flex-col gap-16">
      ...
      {/* Divider between wheels and wheelsLanguages */}
      {data.wheels && data.wheelsLanguages && <div className="w-full h-px bg-stone-200 my-4" />}
        ...
        >
          ...
          {data.wheelsLanguages.map((item, langIndex) => (
            ...
            >
                  ...
                  <AnimatedText delay={0.3}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      {item.list.map((list_item, listIndex) => (
                        ...
                        >
                          <h4
                            className={`text-base font-medium text-stone-700 uppercase tracking-wider ${list_item.heading.tip || list_item.items ? "mb-3" : ""}`}
                          >
                          ...
                          {list_item.heading.tip && (
                            <p
                              className={`text-sm text-stone-500 ${list_item.items ? "mb-3" : ""}`}
                            >
                              {list_item.heading.tip}
                            </p>
                          )}
              ...
              {item.specialNotes && (
                <>
                  <div className="w-full h-px bg-stone-200 mt-8" />
                  <div className="flex flex-col gap-8 mt-8">
                    {item.specialNotes.map((noteItem, noteIndex) => (
                      <React.Fragment key={noteIndex}>
                        {/* Divider before each note except the first */}
                        {noteIndex > 0 && <div className="w-full h-px bg-stone-200" />}
                        <div className="flex flex-col lg:flex-row lg:justify-between gap-8 lg:gap-16 items-start">
                          <div
                          ...
                          >
                            <div className="flex items-baseline gap-4">
                              <span className="text-3xl font-light text-stone-400">
                                {String((data.wheels?.length || 2) + noteIndex + 2).padStart(
                                  2,
                                  "0",
                                )}
                              </span>
              ...
              {langIndex < (data.wheelsLanguages?.length || 0) - 1 && (
                <div className="w-full h-px bg-stone-200 mt-8" />
              )}
            </AnimatedSection>
          ))}
